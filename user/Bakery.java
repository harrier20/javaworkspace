...
public class Bakery extends Page {
	UserMain userMain;
	String[] bakery_img;
	...
	public Bakery(UserMain userMain) {
		super(userMain);
		p_center = new JPanel();
...
//		System.out.println(prList.size());
		...
		bt_menu = new CustomButton[bakery_img.length]; // 가져온 이미지길이만큼 버튼 생성
		// System.out.println(bubbleTea_img.length);
		for (int i = 0; i < bakery_img.length; i++) {
...
//			bt_menu[i] = new CustomButton(ImageIcon(userMain.get));
			bt_menu[i] = new CustomButton(
					new ImageIcon(this.getClass().getClassLoader().getResource("data/" + bakery_img[i])));
...
//					userMain.la_total.setText(Integer.toString(Integer.parseInt(userMain.la_total.getText()) + prList.get(n).getProduct_price())); //총 금액 변경
	...
	public void bringImg() {
		...
		bakery_img = new String[prList.size()];

		for (int i = 0; i < prList.size(); i++) {
			menu_id[i] = prList.get(i).getProduct_id();
			bakery_img[i] = prList.get(i).getFilename();
		}
	}
}
